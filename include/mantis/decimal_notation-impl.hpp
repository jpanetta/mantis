 ...
 */
#ifndef MANTIS_DECIMAL_NOTATION_IMPL_H_
#define MANTIS_DECIMAL_NOTATION_IMPL_H_

#include "mantis/decimal_notation.hpp"

namespace mantis {

inline std::string DecimalNotation::ToString() const {
  std::string s;
  if (!positive) {
    s += '-';
  }
...
inline DecimalNotation& DecimalNotation::FromString(const std::string& rep) {
  positive = true;
  exponent = 0;
  digits.clear();
  if (rep.empty()) {
...
#endif  // ifndef MANTIS_DECIMAL_NOTATION_IMPL_H_
